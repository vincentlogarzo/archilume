...
def main():
    ...
    print("=" * 80)
    print("ARCHILUME - Winter Solstice Sunlight Exposure Analysis")
    print("=" * 80)
    print()
    ...
    image_resolution                    = 2048          # Image size in pixels to be rendered
        # TODO: future implementation to utilise polygon masks for more efficient processingof output results on much largers datasets with smaller time steps. 
    ...
    # 5a: Generate Area of Interest (AOI) files
    ...
    # 5b: Generate Working plane data (WPD) and .csv results for sunlit areas
        # FIXME : the below class is under construction
    results_processor = ResultsProcessor(
        rendering_commands              = renderer.rpict_direct_sun_commands,
    ...
    results_processor.sunlight_sequence_wpd_extraction()

    phase_timings["  5b: Generate WPD"], sub_phase_start = time.time() - sub_phase_start, time.time()

   # 5c: Stamp results onto images and generate combined gifs
    ...
    print("Phase 6: Packaging Final Results...")
    ...
    print("-" * 80)
    print(f"{'Total Runtime':<30} {total_runtime:>8.2f}s  ({total_runtime/60:>5.1f} min)")
    print("=" * 80)

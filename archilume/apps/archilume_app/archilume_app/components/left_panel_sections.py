...
def _floor_plan_body() -> rx.Component:
    ...
    return rx.box(
        ...
        # Resize by Rectangle (calibration rect)
        rx.flex(
            rx.icon(tag="square-dashed", size=13),
            rx.text("Resize by Rectangle", style={"font_family": FONT_MONO, "font_size": "11px", "margin_left": "5px"}),
            on_click=EditorState.toggle_calibration_rect,
            style={
                **btn_style,
                "background": rx.cond(EditorState.calib_rect_active, COLORS["btn_on"], "transparent"),
                "opacity": rx.cond(EditorState.overlay_align_mode, "1", "0.5"),
            },
            color=rx.cond(EditorState.calib_rect_active, COLORS["accent"], COLORS["text_dim"]),
        ),

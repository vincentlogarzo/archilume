...
class TestSkyGenerator:
    ...
    @pytest.fixture
    def basic_generator(self, tmp_path):
        """Basic SkyGenerator instance for testing"""
        return SkyGenerator(lat=-37.8136, sky_file_dir=tmp_path)  # Melbourne latitude

    @pytest.fixture
    def generator_with_defaults(self, tmp_path):
        """SkyGenerator with default values"""
        return SkyGenerator(lat=40.7128, sky_file_dir=tmp_path)  # New York latitude
    ...
    def test_positive_latitude(self, tmp_path):
        """Test with positive latitude (Northern Hemisphere)"""
        generator = SkyGenerator(lat=51.5074, sky_file_dir=tmp_path)  # London latitude
    ...
    def test_zero_latitude(self, tmp_path):
        """Test with zero latitude (equator)"""
        generator = SkyGenerator(lat=0.0, sky_file_dir=tmp_path)
    ...
    def test_sky_file_generation_creates_files(self, basic_generator, temp_dir):
        ...
        basic_generator.sky_file_dir = Path(temp_dir)
    ...
    def test_different_date_formats(self, temp_dir):
        ...
        generator = SkyGenerator(lat=0.0, sky_file_dir=temp_dir)
    ...
    def test_minute_increment_functionality(self, temp_dir):
        ...
        generator = SkyGenerator(lat=45.0, sky_file_dir=temp_dir)
    ...
    def test_single_time_point(self, temp_dir):
        ...
        generator = SkyGenerator(lat=30.0, sky_file_dir=temp_dir)
    ...
    def test_latitude_precision_in_gensky_command(self, temp_dir):
        ...
        generator = SkyGenerator(lat=37.123456789, sky_file_dir=temp_dir)  # High precision latitude
    ...
    @pytest.mark.skip(reason="Feb 29 requires a leap year; no year parameter available in generate_sunny_sky_series")
    def test_leap_year_february(self, temp_dir):
        """Test generation for February 29th in a leap year"""
        generator = SkyGenerator(lat=40.0, sky_file_dir=temp_dir)
    ...
    def test_console_output_format(self, basic_generator, temp_dir, capsys):
        ...
        assert "All sky files have been successfully" in captured.out


class TestSkyGeneratorEdgeCases:
    """Test edge cases and error conditions for SkyGenerator"""

    def test_extreme_latitudes(self, tmp_path):
        """Test with extreme latitude values"""
        # North Pole
        generator_north = SkyGenerator(lat=90.0, sky_file_dir=tmp_path / "north")
        assert generator_north.lat == 90.0

        # South Pole
        generator_south = SkyGenerator(lat=-90.0, sky_file_dir=tmp_path / "south")
        assert generator_south.lat == -90.0